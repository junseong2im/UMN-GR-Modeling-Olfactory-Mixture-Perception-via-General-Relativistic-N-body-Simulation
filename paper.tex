...
\author{Junseong Kim\\
\textit{University of Suwon}\\
\texttt{junseong2im@gmail.com}}
\date{}
...
The model tends to underpredict extremely high similarities (true $= 100$), with errors up to 44 points. This pattern is consistent with regression toward the mean: because the training distribution centers around moderate similarity values (mean $\approx 50$), the MSE-optimal prediction for extreme values is biased toward the distributional mean. With only 360 pairs, the number of examples near the extremes is too small to counteract this tendency. This is a general property of regression models on small, non-uniformly distributed datasets, not a UMN-specific limitation. Mid-range similarities (40--60) are predicted most accurately (MAE $< 5$), corresponding to the most densely populated region. Prediction error does not vary systematically with mixture complexity (total components), providing evidence that the physics simulation generalizes across mixture sizes.
...
To assess generalization beyond the Snitz dataset, we performed zero-shot transfer evaluation on the Ravia et al. (2020) mixture similarity dataset (50 pairs). Models trained exclusively on Snitz data (3-seed $\times$ 5-fold, 3-restart) were evaluated on Ravia without any fine-tuning.

\begin{table}[H]
\centering
\caption{Zero-shot transfer: Snitz $\to$ Ravia (15 evaluations).}
\label{tab:transfer}
\begin{tabular}{lcc}
\toprule
...
The transfer correlation is near zero (mean $r = -0.067$), indicating that Snitz-trained models do not generalize to the Ravia dataset. This is attributable to differences at three levels: (a) measurement construct---Snitz collects direct similarity ratings (0--100) while Ravia measures correct fraction in a discrimination task, which are related but distinct psychophysical constructs; (b) molecular composition---the two datasets use different odorant molecules with limited overlap; and (c) experimental conditions---differences in stimulus presentation, concentration normalization, and subject population.

Importantly, this limitation is not specific to UMN. The Snitz et al. (2013) baseline ($r = 0.85$) was also evaluated exclusively on the Snitz dataset, and no published olfactory mixture similarity model has demonstrated cross-dataset generalization to date. The absence of standardized, cross-compatible benchmarks remains a fundamental bottleneck for the field.
...
However, the effect sizes are small ($|r| < 0.15$), indicating that the learned mass captures a nonlinear combination of molecular features rather than being a proxy for any single chemical property. The absence of correlation with molecular weight ($r = +0.058$, $p = 0.413$) confirms that the learned ``mass'' does not correspond to physical mass but encodes interaction strength in the perceptual similarity space. Importantly, the small effect sizes themselves constitute a meaningful finding: the learned mass encodes a multi-dimensional property combination rather than serving as a shortcut proxy for a single descriptor.

\subsubsection{Comparison with GNN Interpretability Methods}
To contextualize these results, we compare with the predominant interpretability approaches in the molecular GNN literature, which are characterized by reliance on post-hoc attribution methods that identify \textit{which} input substructures are important for a prediction, rather than analyzing \textit{what} the learned internal representations chemically encode.

GNNExplainer \cite{ying2019} identifies compact subgraphs and node feature subsets most relevant to a GNN prediction. While effective for identifying ``which atoms matter,'' it does not reveal what the internal hidden representations encode in chemical terms.

Yang et al. \cite{yang2019} analyzed learned molecular representations in D-MPNN (Chemprop), comparing learned fingerprints against engineered descriptors in predictive performance. However, the study did not quantitatively correlate learned hidden dimensions with specific physicochemical properties.

McCloskey et al. \cite{mccloskey2019} applied Integrated Gradients to graph convolution models for binding prediction, finding that highly accurate models can learn spurious correlations rather than genuine binding mechanisms---highlighting that identifying important input features does not guarantee chemically meaningful internal representations.

Jimenez-Luna et al. \cite{jimenezluna2020} provided a comprehensive review of explainable AI in drug discovery, concluding that ``current methods fall short of providing chemical reasoning'' and that most approaches remain at the level of input-feature importance rather than internal-representation semantics.

Rao et al. \cite{rao2022} developed benchmark datasets for quantitative GNN interpretability evaluation, revealing that even state-of-the-art XAI methods achieve only moderate faithfulness scores, and that ground-truth assignments for explainability rely on subjective chemical judgment.
...
The current implementation supports mixtures of up to $\text{MAX\_MOLS} = 20$ molecular components. This is a configurable hyperparameter, not an architectural constraint. The N-body simulation has $O(N^2)$ computational complexity per time step due to pairwise gravitational force computation. For $N = 50$ (a practical upper bound for complex perfumery formulations), this amounts to 1,225 pairwise interactions per step, which remains computationally trivial on modern GPUs (estimated inference time $< 2$ ms per pair at batch size 16). The vectorized implementation computes all pairwise forces simultaneously via tensor broadcasting, requiring no architectural changes to accommodate larger mixtures.

\subsection{Generalization and Overfitting Considerations}
...
First, the coefficient of variation across 25 cross-validation folds is 5.1\% (std/mean = 0.039/0.780), with an interquartile range of $[0.760, 0.808]$. If the model were overfitting to specific training splits, fold-level variance would be substantially higher.

Second, the error analysis (Section~5.7) shows that prediction error does not systematically vary with mixture complexity. Overfitted models typically show degraded performance on out-of-distribution input sizes, which is not observed.

Third, the physics engine uses only 911 parameters (0.3\% of total), providing structural regularization through a physically constrained information bottleneck.

However, the zero-shot transfer experiment (Section~5.10) demonstrates that the model does not generalize to the Ravia dataset (mean $r = -0.067$). This result must be interpreted carefully. Snitz and Ravia measure different psychophysical constructs under different experimental protocols, with different molecular compositions and limited overlap. Transfer failure under such pronounced domain shift does not constitute evidence of overfitting; rather, it reflects the absence of cross-compatible benchmarks. No published mixture similarity model, including the Snitz et al. baseline itself, has demonstrated generalization across datasets with incompatible measurement protocols. Establishing broader generalization requires future datasets with standardized similarity measurement protocols.

\section{Conclusion}

We presented Universe Multi Neural network (UMN), a differentiable N-body gravitational simulation architecture for olfactory mixture similarity prediction. UMN achieves $r = 0.780$ on the Snitz dataset using 5-seed $\times$ 5-fold cross-validation without manual feature engineering. Key findings: (1) the physics engine architecture is near-optimal in its simplest form, (2) multi-restart training is the most effective strategy for navigating the empirically characterized multi-modal loss landscape, (3) contrastive learning and physics-based losses are counterproductive in small-data regimes, and (4) learned physical quantities show partial chemical interpretability through correlations with LogP and HBA, representing a form of internal-representation analysis that complements the post-hoc attribution methods predominant in the GNN literature.

Limitations include the following. (a) The Snitz dataset (360 pairs) is the only publicly available mixture similarity dataset with direct similarity ratings, and the model does not generalize to the Ravia dataset which uses a different experimental protocol (mean transfer $r = -0.067$). (b) The interpretability analysis reveals statistically significant but weak correlations ($|r| < 0.15$). (c) The multi-restart dependency increases total training cost linearly with restart count. Future directions include larger datasets with standardized protocols, deeper interpretability via causal intervention experiments, simulation-GNN ensembles, and concentration-aware models.
...
\newblock In \textit{NeurIPS}, 32:9240--9251, 2019.

\bibitem[McCloskey et al.(2019)]{mccloskey2019}
McCloskey, K., Taly, A., Monti, F., Brenner, M.~P., and Colwell, L.~J.
\newblock Using attribution to decode binding mechanism in neural network models for chemistry.
\newblock \textit{Proceedings of the National Academy of Sciences}, 116(24):11624--11629, 2019.

\bibitem[Jimenez-Luna et al.(2020)]{jimenezluna2020}
Jimenez-Luna, J., Grisoni, F., and Schneider, G.
\newblock Drug discovery with explainable artificial intelligence.
\newblock \textit{Nature Machine Intelligence}, 2(10):573--584, 2020.

\bibitem[Rao et al.(2022)]{rao2022}
Rao, J., Zheng, S., and Yang, Y.
\newblock Quantitative evaluation of explainable graph neural networks for molecular property prediction.
\newblock \textit{Patterns}, 3(12):100628, 2022.
